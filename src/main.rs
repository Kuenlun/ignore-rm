...
fn pick_folder(start: &Path) -> Result<PathBuf, PickerError> {
...
fn discover_topmost<P: AsRef<Path>>(start: P) -> Result<Repository, PickerError> {
...
fn delete_paths<P, I>(base: &Path, paths: I) -> Result<(), PickerError>
where
...
fn wait_for_enter() -> Result<(), PickerError> {
    Input::<String>::new()
